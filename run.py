...
def main():
    ...
    demo_mode = input("\nStart in DEMO MODE (15-day pre-seeded history)? (y/n): ").lower()
    if demo_mode == 'y':
        print("\n--- Running Demo Start Sequence ---")
        demo_script = os.path.join(ROOT_DIR, "tools", "start_demo.py")
        result = subprocess.run([VENV_PYTHON, demo_script])
        if result.returncode != 0:
            print("Demo start failed. Seed data may be missing. Run: python tools/demo_seeder.py")
            sys.exit(1)

    reset = input("\nReset stream files and topics (Hard Reset)? (y/n): ").lower()
    if reset == 'y' and demo_mode != 'y':
        if infra_is_running:
